...
fig1 = px.pie(working_df, names="TARGET",color="TARGET", color_discrete_sequence=["#FF5733", "#39ED0C"], title="Target Distribution (0 = Repaid, 1 = Default)")
...
fig3 = px.histogram(working_df, x="AGE_YEARS",nbins=30, title="Age Distribution",color_discrete_sequence=["#800dec"])
row1_col3.plotly_chart(fig3, use_container_width=True)

# Row 2
row2_col1, row2_col2, row2_col3 = st.columns(3)

fig4 = px.histogram(working_df, x="AMT_INCOME_TOTAL",nbins=30, title="Annual Income Distribution",color_discrete_sequence=["#0bf10b"])
row2_col1.plotly_chart(fig4, use_container_width=True)

fig5 = px.histogram(working_df, x="AMT_CREDIT", nbins=30, title="Credit Amount Distribution",color_discrete_sequence=["#0bf0ce"])
row2_col2.plotly_chart(fig5, use_container_width=True)

fig6 = px.box(working_df, y="AMT_INCOME_TOTAL", title="Income Boxplot",color_discrete_sequence=["#ec11da"])
row2_col3.plotly_chart(fig6, use_container_width=True)

# Row 3
row3_col1, row3_col2, row3_col3 = st.columns(3)

fig7 = px.box(working_df, y="AMT_CREDIT", title="Credit Amount Boxplot",color_discrete_sequence=["#0cec0c"])
row3_col1.plotly_chart(fig7, use_container_width=True)

fig8 = px.histogram(working_df, x="CODE_GENDER", title="Gender Distribution", text_auto=True,color_discrete_sequence=["#f20909"])
row3_col2.plotly_chart(fig8, use_container_width=True)

fig9 = px.histogram(working_df, x="NAME_FAMILY_STATUS",title="Family Status Distribution", text_auto=True,color_discrete_sequence=["#0944f3"])
row3_col3.plotly_chart(fig9, use_container_width=True)

# Row 4 (last graph centered)
row4_col1, row4_col2, row4_col3 = st.columns(3)

fig10 = px.histogram(working_df, x="NAME_EDUCATION_TYPE",title="Education Distribution", text_auto=True,color_discrete_sequence=["#dfee0d"])
...
- Some features have **high missingness** (>50–60%), requiring careful treatment.  
- **Income and credit** distributions are highly skewed, with long tails.
- Defaults are more common in **low-to-mid** income groups.  
- **Age distribution** is fairly normal, Most customers are 25–45 years old.  
""")